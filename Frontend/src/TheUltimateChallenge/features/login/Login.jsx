...
function Login() {
  ...
  // Regex to allow only letters (including accented characters)
  const nameRegex = /^[a-zA-ZÀ-ÿ\s]*$/;

  const handleFirstNameChange = (e) => {
    const value = e.target.value;
    // Only update state if the value matches the regex (letters and spaces only)
    ...
    const fetchTeams = async () => {
      ...
      try {
        ...
        const response = await fetch(
          ...
          }/api/v1/theultimatechallenge/totalteams`,
          {
            method: "POST",
            headers: {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${
          import.meta.env.VITE_BACKEND_BASE_URL
        }/api/v1/theultimatechallenge/createplayer`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
