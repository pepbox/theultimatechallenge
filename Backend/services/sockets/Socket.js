...
function setupSocket(io) {
    io.on('connection', (socket) => {
        ...
        // ─── Manual Verification: Player requests verification ──────────────────
        socket.on('request-manual-verification', async (data, callback) => {
            try {
                const cookies = socket.handshake.headers.cookie;
                if (!cookies) {
                    if (callback) callback({ success: false, error: 'No cookies found' });
                ...
                if (questionStatus.status === 'done') {
                    if (callback) callback({ success: false, error: 'Question already completed' });
                    return;
                }
                ...
                // Build question info for the admin notification
                const questionDoc = questionStatus.question;

                // Emit all-teams-data to admin (updates dashboard table + FAB badge)
                const allTeams = await Team.find({ session: session._id }).populate({
                    path: 'questionStatus.question',
                    model: 'Question'
                });
                ...
                if (admin && admin.socketId) {
                    ...
                    // Also emit the dedicated notification event for the FAB modal
                ...
                // Emit updated team-data to all players in team
                ...
                teamSocketIds.forEach(sid => io.to(sid).emit('team-data', teamPayload));
        ...
        socket.on('approve-manual-verification', async (data, callback) => {
            try {
                ...
                // Emit team-data and approval event to players
                const updatedTeam = await Team.findById(team._id).populate({ path: 'questionStatus.question', model: 'Question' });
                const qData2 = updatedTeam.questionStatus.map(q => ({
                    id: q.question._id,
                    text: q.question.text,
        ...
        socket.on('reject-manual-verification', async (data, callback) => {
            try {
                ...
                // Revert to attending so player can request again
                await Team.updateOne(
                    { _id: team._id, 'questionStatus.question': data.questionId },
                    {
                        $set: { 'questionStatus.$.status': 'attending' },
                    }
                );
