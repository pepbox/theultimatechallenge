  ...
  return (
    <div className="w-[305px] py-4 bg-[#8C8C8C1A] rounded-[20px] font-sans ">
      ...
      <div className="flex gap-2 w-[100%] px-4 mt-3">
        <button onClick={copyToClipboard} className="border grow h-[34px] rounded-[12px] cursor-pointer">
          {copyLink ? "Copied!" : "Copy Link"}
        </button>
        <button onClick={()=>setViewModalOpen(true)} className="bg-black grow text-white h-[34px] cursor-pointer rounded-[12px]">
          View
        </button>
      </div>
      {viewModalOpen && <SuccessPopup sessionData={sessionData} onClose={() => setViewModalOpen(false)} />}
    </div>
  );
}
