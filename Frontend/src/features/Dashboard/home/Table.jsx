...
function Table({ gameStatusRef, onLevelChange, pendingStatusChange, transactionsEnabled }) {
    ...
    socket.emit("request-all-teams-data", (response) => {
      if (response.success) {
        ...
        const formattedData = response.data.teams.map((team) => ({
          ...
          questions: team.questions, // Include full question data for modals
          rawData: team // Include raw data for reference
        }));
        setTeamData(formattedData);

        // Update selectedTeam if StatusModal or SubmissionModal is open
        if ((showStatusModal || showSubmissionModal) && selectedTeam) {
          const updatedTeam = formattedData.find((team) => team.id === selectedTeam.id);
          if (updatedTeam) {
            // Only update if questions or score have changed
            if (
              JSON.stringify(updatedTeam.questions) !== JSON.stringify(selectedTeam.questions) ||
              updatedTeam.score !== selectedTeam.score
            ) {
              console.log(`Updating selectedTeam ${selectedTeam.id} due to data change`);
              setSelectedTeam(updatedTeam);
            }
          } else {
            // Close modal if team no longer exists
            console.log(`Team ${selectedTeam.id} no longer exists, closing modal`);
  ...
  }, [socket, gameStatusRef, onLevelChange, pendingStatusChange, showStatusModal, showSubmissionModal, selectedTeam]);
          ...
          <tbody>
            {teamData.map((team, index) => (
              <tr key={team.id} className="even:bg-gray-50 text-center rounded-2xl">
                <td>
                  <div className="flex justify-center items-center h-16 rounded-l-2xl text-[14px]">{team.name}</div>
                </td>
                <td>
